 ...
 * 辅料 新的价格取数逻辑201809
 */
select distinct a.faidnumber,b.fnumber
	from t_costcalculatebd_entry1 	a 
join t_icitem 					b	on a.faidnumber  = b.fitemid  
where b.fitemid not in (select w.fitemid from (select b.fitemid ,sum(b.fstdamount) as funtaxamount, sum(b.fqty) as fqty,case  
...
and b.fitemid not in (select  x.fitemid from t_supplyentry x where x.fdisabledate >  getdate()  and x.fprice > 0)

update t_bos200000025entry set fprice =  b.fplanprice
from t_bos200000025entry a join t_icitem b on a.fbase=b.fitemid
/*外购物料*/

declare @fitemid int ,@fname varchar(30) ,@fmodel varchar(30),@funitid int
	,@planprice decimal(20,3),@findex int
declare cursor0 cursor  for
select distinct a.fitemid,a.fname,a.fmodel,a.funitid,a.fplanprice
	from t_icitem 					a	
...
open cursor0
fetch next from cursor0 into @fitemid ,@fname,@fmodel ,@funitid,@planprice
set @findex=3
while @@fetch_status =0
begin
	insert into t_bos200000025entry(fid,findex,fbase,ftext,ftext1,fbase1,fprice,fdate2,fnote)
	values(1001,@findex+1,@fitemid ,@fname,@fmodel ,@funitid,@planprice,'2019-01-01','system')
