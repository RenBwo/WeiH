 ...
 */
			...
			,double length,double capacity0) 	throws SQLException	
	{
		...
		+firstitemid+" and a.finterid = "+finterid
		...
		+ " and  a.sn not in (select a.sn  from BDBomMulExpose 	a  "
				...
				+ " on a.fitemid = b.fitemid  and a.firstitemid = "+firstitemid
				+" and a.finterid = "+finterid+" and b.fbomskip = 1058 "
		...
		+ "(case t4.foperid when 40336 then " + capacity0         	/*钎焊炉工艺 40336*/
		+ " when 40494 then round(t11.fpressrate*60/"+length+ ",4)" /*扁管压出工艺40494*/
		+ " when 40495 then (case when " + length*1000 +" < 400 "	/*扁管切断工艺 40495*/
				+ " then 7800 "			
				+ " when " + length*1000 +">=400 and "+length*1000 +"< 600 then 6600"
				+ " when " + length*1000 +">=600 and "+length*1000 +"< 800 then 5400"
				+ " when " + length*1000 +">=800 and "+length*1000 +"< 1000 then 4200"
		...
		+ " isnull("
			+ " (case isnull((select 1 from t_icitem where fitemid = t1.fitemid "
				...
				+ " then t8.fqty/"+capacity0
				+ " when (t4.foperid = 40494 and t8.faidname like '%锌丝%' )"
				+ " then t11.fqtyzn/1000*"+length
				+ " when (t4.foperid = 40142 and "+length + "<=0.2 ) then t8.fqty/2 else t8.fqty "
				+ "end)"
			+ ",0) as famtAdi "		
		+ " into #t_adi "		
		+ " from BDBomMulExpose 				t1  "
		...
		+ " and (t8.fcolor = 0 or (t8.fcolor = t2.f_135 and t2.fitemid = ("
			...
			+firstitemid+" and t34.finterid = "+finterid
		...
		+ " (case isnull((select 1 from t_icitem where fitemid = t1.fitemid"
			...
			+ " when 1 then round(t1.fqty*(100-t1.fscrap)/100/t1.fqtyper,0) else t1.fqty end)"
			+ "*t4.fentryselfz0237*"
			+ "(case t4.foperid when 40494 then round(t11.famtmodel*"+length
		...
		+firstitemid+" and t1.finterid = "+finterid	
		+ " join t_Routing t3 on t3.finterid = t1.froutingid "		
		+ " join t_routingoper t4 on t3.finterid = t4.finterid "		
		...
		+ " select "+firstitemid+ ","+finterid+ ",t1.foperid,t1.fopersn,t1.fnumber"
		...
		String cleanLabourAndMake=";delete from t_BDLabourAndMake where fproditemid = "
     	 +firstitemid +" and finterid < "+finterid ;
