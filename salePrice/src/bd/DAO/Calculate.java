...
*/
	public void CostLabourAndMake(
			...
			,double EnergyPrice,int firstitemid,int finterid
			,String currentyear,String currentperiod
			,double length,double capacity0) 	throws SQLException	
	{	/* 直接人工		 */
		String sql0 = ";select d.foperid,d.fopersn,b.fnumber ,b.fname as assyname,"
		+ " (case isnull((select 1 from t_icitem where fitemid = a.fitemid and fname like '扁管盘料'),0) "
		...
		+ "*d.fpiecerate*(case d.fentryselfz0236 when 0 then 1 else d.fentryselfz0236 end)*d.fentryselfz0237 as amtpay,"
		+k1 + "*(case isnull((select 1 from t_icitem where fitemid = a.fitemid and fname like '扁管盘料'),0) "
		+ " when 1 then round(a.fqty*(100-a.fscrap)/100/a.fqtyper,0) else a.fqty end)"
		+ "*d.fpiecerate*(case d.fentryselfz0236 when 0 then 1 else d.fentryselfz0236 end)*d.fentryselfz0237 as amtassure,"
		+ " (case isnull((select 1 from t_icitem where fitemid = a.fitemid and fname like '扁管盘料'),0) "
		+ " when 1 then round(a.fqty*(100-a.fscrap)/100/a.fqtyper,0) else a.fqty end)"
		+ "*d.fpiecerate*(case d.fentryselfz0236 when 0 then 1 else d.fentryselfz0236 end)*d.fentryselfz0237*(1+ "+k1+ ")*"+k2+ " as cost_worker "
		+ " into #t_costworker "
		+ " from BDBomMulExpose 	a "
		+ " join t_icitem 			b 	on a.fitemid = b.fitemid and a.firstitemid = "+firstitemid+" and a.finterid = "+finterid
		...
		+ " and  a.sn not in (select a.sn  from BDBomMulExpose 	a  "
				+ "	join t_icitem b on a.fitemid = b.fitemid  and b.ferpclsid <> 1 and a.firstitemid = "+firstitemid+" and a.finterid = "+finterid
				+ " join (select a.sn from BDBomMulExpose  a join icbom b  "
					+ " on a.fitemid = b.fitemid  and a.firstitemid = "+firstitemid+" and a.finterid = "+finterid+" and b.fbomskip = 1058 "
		...
		+ " isnull("
		+ " (case isnull((select 1 from t_icitem where fitemid = t1.fitemid and fname like '扁管盘料'),0) "
		+ " when 1 then round(t1.fqty*(100-t1.fscrap)/100/t1.fqtyper,0) else t1.fqty end)" /*零部件数量*/
		+ "*t4.fentryselfz0237*t6.fpower*0.4*"+EnergyPrice+ "/"
		+ "(case t4.foperid when 40336 then " + capacity0              				/*钎焊炉工艺 40336*/
		+ " when 40494 then round(t11.fpressrate*60/"+length+ ",4)" 					/*扁管压出工艺40494*/
		+ " when 40495 then (case "														/*扁管切断工艺 40495*/
				...
				+ " else  2400 end  )" 								
		+ " else round( t6.fcapacity/(case t4.fentryselfz0236 when 0 then 1 else t4.fentryselfz0236 end),4) end) "/*其余工艺*/
		+ " ,0 ) as famtpower,  " /*电费*/
		+ " isnull("
		+ " (case isnull((select 1 from t_icitem where fitemid = t1.fitemid and fname like '扁管盘料'),0) "
		+ " when 1 then round(t1.fqty*(100-t1.fscrap)/100/t1.fqtyper,0) else t1.fqty end)" /*零部件数量*/
		...
		+ "(case t4.foperid when 40336 then " + capacity0              				/*钎焊炉工艺 40336*/
		+ " when 40494 then round(t11.fpressrate*60/"+length+ ",4)" 					/*扁管压出工艺40494*/
		+ " when 40495 then (case when " + length*1000 +" < 400 then 7800 "			/*扁管切断工艺 40495*/
				+ " when " + length*1000 +">=400 and "+length*1000 +"< 600 then 6600"
				+ " when " + length*1000 +">=600 and "+length*1000 +"< 800 then 5400"
				+ " when " + length*1000 +">=800 and "+length*1000 +"< 1000 then 4200"
				+ " else  2400 end )" 								
		...
		+ " when (t4.foperid = 40336 and (t8.faidname like '%钎剂%' or  t8.faidname like'%液氮%')) then t8.fqty/"+capacity0
		+ " when (t4.foperid = 40494 and t8.faidname like '%锌丝%' ) then t11.fqtyzn/1000*"+length
		+ " when (t4.foperid = 40142 and "+length + "<=0.2 ) then t8.fqty/2 else t8.fqty end),0) as famtAdi "		
		+ " into #t_adi "		
		+ " from BDBomMulExpose 				t1  "
		+ " join t_icitem 						t2 	on t1.fitemid = t2.fitemid  and t1.firstitemid = "+firstitemid+" and t1.finterid = "+finterid
		+ " join t_Routing 						t3	on t3.finterid = t1.froutingid "		
		...
		+ " and (t8.fcolor = 0 or (t8.fcolor = t2.f_135 and t2.fitemid = (select t34.fitemid "
		+ " 		from t_icitemcore t33 join BDBomMulExpose t34 on t34.firstitemid = "+firstitemid+" and t34.finterid = "+finterid
		...
		String sql4 =";select foperid,fopersn,fnumber,assyname,sum(fqty) as fqty,opername,fpiecerate,frate,"
		+ " fmakeqty,sum(amtpay) as amtpay, sum(amtassure) as amtassure,sum(cost_worker) as costworker "		
		+ " into #t_20 "		
		+ " from #t_costworker "		
		...
		+ " ;select foperid,fopersn,fnumber,assyname,sum(fqty) as fqty,opername,fpiecerate,frate,"
		...
		String sql5 =";insert into t_BDLabourAndMake( fproditemid,finterid,foperid,fopersn,fnumber,assyname,fqty,fopername,fpiecerate,"
		+ " frate,fmakeqty,amtpay,amtassure,costworker,fmatpower,fdepr,famtadi,famtmodel) "		
		+ " select "+firstitemid+ ","+finterid+ ",t1.foperid,t1.fopersn,t1.fnumber,left(t1.assyname,30),t1.fqty,t1.opername,t1.fpiecerate,"
		+ " t1.frate,t1.fmakeqty,t1.amtpay,t1.amtassure,t1.costworker,t2.fmatpower,t2.fdepr,t3.famtadi,t4.famtmodel "
		+ " from #t_20 t1 "
		+ " left join #t_21 t2 on t1.foperid=t2.foperid and t1.fnumber = t2.fnumber and t1.fopersn = t2.fopersn "		
		+ " left join #t_22 t3 on t1.foperid=t3.foperid and t1.fnumber = t3.fnumber and t1.fopersn = t3.fopersn "		
		+ " left join #t_23 t4 on t1.foperid=t4.foperid and t1.fnumber = t4.fnumber and t1.fopersn = t4.fopersn "
		
		+ " ;drop table #t_model;drop table #t_adi;drop table #t_machine;drop table #t_costworker"
